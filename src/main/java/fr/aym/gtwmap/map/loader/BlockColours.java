...
public class BlockColours {
    ...
    public int getBiomeColour(IBlockState BlockState, int biomeId) {
        ...
        if (biome != null) {
            biomeName = ObfuscationReflectionHelper.getPrivateValue(Biome.class, biome, "field_76791_y");
            ...
            case NORMAL:
                ...
                try {
                    int renderColour = block.getMapColor(
                            block.getStateFromMeta(Integer.parseInt(meta) & 0xf), null, null).colorValue; //FIXME POTENTIAL NPE
    ...
    public void saveBiomes(Writer fout) throws IOException {
        ...
        for (Entry<String, BiomeData> entry : this.biomeMap.entrySet()) {
    ...
    public void saveBlocks(Writer fout) throws IOException {
        ...
        for (Entry<String, BlockData> entry : this.bcMap.entrySet()) {
    ...
    public boolean CheckFileVersion(File fn) {
        ...
        return lineData.equals(String.format("version: %s", "ared"));
