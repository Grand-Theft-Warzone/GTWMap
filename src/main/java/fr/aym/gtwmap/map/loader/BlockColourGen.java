                ...
                if (block != null && block.getRenderType(block.getDefaultState()) != EnumBlockRenderType.INVISIBLE) {
                    ...
                    try {
                        if (block instanceof PaintBlockBase) {
                            int i = ((PaintBlockBase) block).getColour().getId();
                            icon = Minecraft.getMinecraft().getTextureMapBlocks().getAtlasSprite(FurenikusRoads.MODID + ":blocks/paint_" + FRBlocks.col.get(i).getName());
                        } else {
                    ...
                    if (icon != null) {
                        ...
                        if ((u1 == u1Last) && (u2 == u2Last) && (v1 == v1Last) && (v2 == v2Last)) {
                            ...
                            // blocks on the map
                            if (Block.REGISTRY.getNameForObject(block)
                                    .getNamespace()
